...
def draw_bev(ax, points, boxes, scores, labels, class_names, pc_range, score_thresh,
    ...
    # Pred boxes (sólido) – verifica se boxes não é None
...
def main():
    ...
    # ===== BYPASS para aceitar lista de samples =====
    inferencer._inputs_to_list = lambda inputs, **kwargs: inputs
    # ===== BYPASS do postprocess para evitar erro de box_mode_3d =====
    ...
    for idx in range(total):
        ...
        # ===== GARANTIR QUE CADA CÂMERA TENHA lidar2cam e cam2img =====
    for cam in sample["images"]:
        if 'lidar2cam' not in sample["images"][cam]:
            # Tenta obter de sample['lidar2cam'] se for uma lista
            if 'lidar2cam' in sample and isinstance(sample['lidar2cam'], list):
                cam_list = list(sample["images"].keys())
                idx = cam_list.index(cam)
                if idx < len(sample['lidar2cam']):
                    sample["images"][cam]['lidar2cam'] = sample['lidar2cam'][idx]
                else:
                    sample["images"][cam]['lidar2cam'] = np.eye(4, dtype=np.float32)
            else:
                sample["images"][cam]['lidar2cam'] = np.eye(4, dtype=np.float32)
        if 'cam2img' not in sample["images"][cam]:
            sample["images"][cam]['cam2img'] = np.eye(3, dtype=np.float32)

        # ===== CORREÇÃO: forçar box_type_3d para a classe =====
        ...
        if 'annos' in sample:
            annos = sample['annos']
            # Tenta diferentes nomes de chave
            if 'bboxes_3d' in annos:
                gt_boxes = annos['bboxes_3d']
                gt_labels = annos.get('labels_3d', np.zeros(len(gt_boxes), dtype=np.int32))
            elif 'gt_bboxes_3d' in annos:
                gt_boxes = annos['gt_bboxes_3d']
                gt_labels = annos.get('gt_labels_3d', np.zeros(len(gt_boxes), dtype=np.int32))
            elif 'boxes_3d' in annos:
                gt_boxes = annos['boxes_3d']
                gt_labels = annos.get('labels_3d', np.zeros(len(gt_boxes), dtype=np.int32))
            else:
                print(f"  Aviso: chave de caixas não encontrada em annos. Chaves disponíveis: {list(annos.keys())}")

        # Dados para desenho
        l2i = np.asarray(sample.get("lidar2image", [np.eye(4)])[0], np.float32)
        aug = np.asarray(sample.get("img_aug_matrix", [np.eye(4)])[0], np.float32)
